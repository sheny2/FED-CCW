...
# grid for federated CCW, pooled CCW, direct g-comp, and CC-gcomp.
#
# The common axis is correct / coarse_L / no_tv / tree. `tree` replaces the
# logistic CCW denominator and g-computation outcome models with rpart
# classification trees; the CCW numerator remains logistic and the L
# transition models remain correctly specified.
#
...
#
# Grid: 3 tau x 3 beta_trt x 3 scenario x 4 misspec = 108 tasks.
#
# Usage (invoked by job.sh):
#   Rscript run_sim_array.R <task_id>    (task_id in 1..108)
...
if (!requireNamespace("rpart", quietly = TRUE))
  stop("Package 'rpart' is required for the tree scenario.")
...
if (any(!ok))
  warning(sprintf("[task %d] %d replicate(s) failed and were dropped.",
                  task_id, sum(!ok)))
if (!any(ok))
  stop(sprintf("[task %d] all replicates failed.", task_id))
