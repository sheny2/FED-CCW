...
# the trimmed two-method study (fed-CCW vs CC-gcomp).
#
...
source("params.R")   # DEFAULT_CONF_MULTS, GCOMP_MISSPEC_SPECS
...
method_levels <- c("fed_ccw_tvipcw", "ccw_gcomp")
method_labels <- c("Federated CCW", "CC-Gcomp")
...
misspec_levels <- intersect(GCOMP_MISSPEC_SPECS, unique(results$misspec))
message(sprintf("Specifications present: %s",
                paste(misspec_levels, collapse = ", ")))
missing_spec <- setdiff(GCOMP_MISSPEC_SPECS, unique(results$misspec))
