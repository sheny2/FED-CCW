#!/usr/bin/env Rscript
# Combine all ArraySim5-1 Five Methods results and summarize performance.
...
scenario_levels <- names(DEFAULT_CONF_MULTS)
...
for (est in plot_estimands) {
  ...
  p <- ggplot(d, aes(x = tau_f, y = bias, fill = method)) +
    ...
    labs(
      title = paste0("Estimation Performanc", est),
      subtitle = paste0(
      ...
      y = "Bias", fill = "Method",
      caption = expression("Columns show treatment-effect coefficient "~beta[trt]~"; rows show confounding strength.")
    ) +
