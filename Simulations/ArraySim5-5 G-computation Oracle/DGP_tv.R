...
simulate_multisite_tv <- function(K          = length(DEFAULT_SITE_SIZES),
  ...
  if (nrow(patient_mix) != K ||
      !all(c("x1_mean", "x1_sd", "x2_prob") %in% names(patient_mix)))
    stop("patient_mix must contain one valid row per site.")
  ...
  sites <- lapply(seq_len(K), function(k) {
    ...
    do.call(simulate_site_tv, c(list(
                     ...
                     site_id = k,
                     seed    = base_seed + k,
                     x1_mean = patient_mix$x1_mean[k],
                     x1_sd   = patient_mix$x1_sd[k],
                     x2_prob = patient_mix$x2_prob[k],
...
compute_truth_ccw_tv <- function(N          = 2e6,
  ...
  if (nrow(patient_mix) != length(site_sizes) ||
      !all(c("x1_mean", "x1_sd", "x2_prob") %in% names(patient_mix)))
    stop("patient_mix must contain one valid row per site.")
  ...
  x1 <- rnorm(
    N,
    mean = patient_mix$x1_mean[site],
    sd = patient_mix$x1_sd[site]
  )
  x2 <- rbinom(N, 1, prob = patient_mix$x2_prob[site])
  ...
  list(
    ...
    patient_mix = patient_mix
  )
}


# =============================================================
# compute_truth_gcomp_tv
#   Parametric Monte Carlo g-computation oracle for the two explicit
#   grace-period strategies used in ArraySim5-5.
#
#   g = 1 (initiate within tau): before tau, initiation follows the TRUE
#       site-specific natural initiation hazard conditional on X and current
#       L. Anyone still untreated in interval tau is deterministically
#       initiated in tau, so every patient follows the strategy.
#   g = 0 (do not initiate): treatment is withheld throughout follow-up.
#
#   L is unaffected by treatment in this DGP, so a common simulated L path is
#   valid for both arms. Rather than drawing binary event times, the function
#   recursively averages each patient's true conditional survival
#   probability. This Rao-Blackwellized g-formula has much less Monte Carlo
#   noise than simulating deaths and is not based on IPCW or artificial
#   censoring. Site membership and baseline X are drawn from the exact
#   scenario-specific target-population mixture.
# =============================================================
compute_truth_gcomp_tv <- function(N          = 2e6,
                                   tau        = DEFAULT_TAU,
                                   t_star     = DEFAULT_TSTAR,
                                   beta_event = DEFAULT_BETA_EVENT,
                                   beta_init  = DEFAULT_BETA_INIT,
  ...
  if (length(N) != 1L || !is.finite(N) || N <= 0 || N %% 1 != 0)
    stop("N must be one positive integer.")
  if (tau < 1L || tau > t_star)
  ...
  x1 <- rnorm(N, patient_mix$x1_mean[site], patient_mix$x1_sd[site])
  x2 <- rbinom(N, 1, patient_mix$x2_prob[site])
  init_intercept_i <- unname(init_intercepts[site])

  L1_prev <- numeric(N)
  L2_prev <- numeric(N)
  initiated_g1 <- rep(FALSE, N)
  initiation_interval_g1 <- rep(M + 1L, N)
  surv_g1 <- rep(1, N)
  surv_g0 <- rep(1, N)
  S1 <- numeric(M + 1L)
  S0 <- numeric(M + 1L)
  ...
  for (m in seq_len(M)) {
    ...
    if (m < tau) {
      eligible <- !initiated_g1
      if (any(eligible)) {
        lin_init <- init_intercept_i +
          beta_init["x1"] * x1 + beta_init["x2"] * x2 +
          beta_init["L1"] * L1_m + beta_init["L2"] * L2_m
        fire <- eligible & (runif(N) < plogis(lin_init))
        initiated_g1[fire] <- TRUE
        initiation_interval_g1[fire] <- m
      }
    } else if (m == tau) {
      # The intervention completes the initiate-within-tau strategy.
      fire <- !initiated_g1
      initiated_g1[fire] <- TRUE
      initiation_interval_g1[fire] <- m
    }

    common_lp <- beta_event["int"] +
      beta_event["x1"] * x1 + beta_event["x2"] * x2 +
      beta_event["L1"] * L1_m + beta_event["L2"] * L2_m +
      beta_event["L1lag"] * L1_prev + beta_event["L2lag"] * L2_prev

    hazard_g1 <- plogis(common_lp + beta_event["trt"] * as.integer(initiated_g1))
    hazard_g0 <- plogis(common_lp)
    surv_g1 <- surv_g1 * (1 - hazard_g1)
    surv_g0 <- surv_g0 * (1 - hazard_g0)
    S1[m + 1L] <- mean(surv_g1)
    S0[m + 1L] <- mean(surv_g0)
  ...
  list(
    ...
    OR = (psi1 / (1 - psi1)) / (psi0 / (1 - psi0)),
    RMST1 = RMST1, RMST0 = RMST0, RMST_diff = RMST1 - RMST0,
    N_used = N, site_N = site_N, site_weights = site_prob,
    init_intercepts = init_intercepts, patient_mix = patient_mix,
    strategy = c(
      g1 = "true natural initiation before tau; force initiation at tau",
      g0 = "withhold treatment throughout follow-up"
    ),
    oracle_type = "parametric Monte Carlo g-computation",
    initiation_time_distribution_g1 =
      tabulate(initiation_interval_g1, nbins = M) / N
  )
}
