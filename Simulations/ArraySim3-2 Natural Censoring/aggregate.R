...
# the natural-censoring five-method study.
#
...
#   fill    = method
#
# t_star is fixed by the runner, so it is reported in the subtitle.
#
...
method_levels <- c("fed_ccw_tvipcw", "pooled_ccw_tvipcw", "ccw_gcomp",
                   "tvipcw_no_clone", "perprotocol_naive")
method_labels <- c("Federated CCW", "Pooled CCW", "CC-Gcomp",
                   "IPCW (no cloning)", "Per-protocol (no IPTW)")
method_colors <- c("Federated CCW"          = "#2c7fb8",
                   "Pooled CCW"             = "#41b6c4",
                   "CC-Gcomp"               = "#568203",
                   "IPCW (no cloning)"      = "#f5a623",
...
for (est in plot_estimands) {
  ...
  p <- ggplot(d, aes(x = tau_f, y = bias, fill = method)) +
    ...
    labs(
      title    = sprintf("Bias of %s by method, under natural censoring (%d reps/cell)",
                         est, n_reps_cell),
