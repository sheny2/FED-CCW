#!/usr/bin/env Rscript
# Lightweight preflight for ArraySim5-3 Site Practice Heterogeneity.
...
init_intercepts_test <- get_init_intercepts(heterogeneity_test)
mix_test <- DEFAULT_PATIENT_MIX
...
# Patient mix is intentionally homogeneous, isolating site practice effects.
...
stopifnot(
  max(abs(empirical_mix$x1_mean)) < 0.15,
  max(abs(empirical_mix$x1_sd - 1)) < 0.15,
  max(abs(empirical_mix$x2_prob - 0.4)) < 0.06,
  abs(mean(dat$x1)) < 0.10,
  abs(mean(dat$x2) - 0.40) < 0.04
)
practice_spread <- vapply(SITE_INIT_INTERCEPTS, function(x) diff(range(x)),
                          numeric(1))
zero_cov_prob <- lapply(SITE_INIT_INTERCEPTS, plogis)
stopifnot(
  all(diff(practice_spread) > 0),
  all(vapply(SITE_INIT_INTERCEPTS, mean, numeric(1)) == -3),
  all(vapply(zero_cov_prob, function(x) all(diff(x) > 0), logical(1)))
)
message(paste(
  "PASS: baseline patient mix is homogeneous; treatment-initiation intercept",
  "spread increases from low to moderate to high around a fixed center."
))
...
pooled_ccw <- run_pooled_ccw_tvipcw(dat, tau, t_star)
aligned_fields <- c(
...
message(paste(
  "PASS: Fed-CCW and fully site-stratified pooled CCW agree across three",
  "heterogeneous-practice sites (point estimates and model-based SEs)."
))
...
stopifnot(
  nrow(grid) == 27L,
  identical(unique(grid$practice_heterogeneity), names(SITE_INIT_INTERCEPTS))
)
message("PASS: array grid contains 27 cells and all three heterogeneity levels.")
message("All ArraySim5-3 Site Practice Heterogeneity preflight checks passed.")
