...
req_cols <- c("method", "estimand", "estimate", "truth", "bias", "rep",
              "tau", "beta_trt", "practice_heterogeneity", "t_star",
...
heterogeneity_levels <- names(SITE_INIT_INTERCEPTS)
...
summ <- pdat %>%
  group_by(method, estimand, tau, beta_trt, practice_heterogeneity,
  ...
  arrange(estimand, method, practice_heterogeneity, beta_trt, tau) %>%
  as.data.frame()
