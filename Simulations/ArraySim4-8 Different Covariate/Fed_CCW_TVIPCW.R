...
.tv_init_hazard <- function(P, hnum = NULL,
  ...
  if (!all(covariates %in% WEIGHT_COVARIATES))
    stop("Unknown initiation-model covariate(s): ",
         paste(setdiff(covariates, WEIGHT_COVARIATES), collapse = ", "))
  multi_site <- nlevels(pi_df$site) > 1L
  den_terms <- c("m", if (multi_site) "site", covariates,
                 if (multi_site && site_interactions && length(covariates))
                   paste0("site:", covariates))
  den_formula <- reformulate(den_terms, response = "y")
...
run_fed_ccw_tvipcw <- function(dat, tau, t_star, trunc = DEFAULT_TRUNC,
                               site_covariates = NULL) {
  hnum <- .tv_common_num_hazard(dat, t_star)
  site_data <- split(dat, dat$site)
  if (is.null(site_covariates))
    site_covariates <- rep(list(WEIGHT_COVARIATES), length(site_data))
  if (length(site_covariates) != length(site_data))
    stop("site_covariates must have one entry per site.")
  stats <- Map(function(d, covs) local_ccw_tvipcw(
    d, tau = tau, t_star = t_star, trunc = trunc, hnum = hnum,
    covariates = covs), site_data, site_covariates)
...
run_pooled_ccw_tvipcw <- function(dat, tau, t_star, trunc = DEFAULT_TRUNC,
                                  covariates = WEIGHT_COVARIATES) {
  hnum <- .tv_common_num_hazard(dat, t_star)
  # The pooled nuisance model includes a site fixed effect, preserving the
  # deliberately different site-specific treatment prevalences.
  stats <- list(local_ccw_tvipcw(dat, tau = tau, t_star = t_star,
...
run_fed_ipw_nocloning <- function(dat, tau, t_star,
  ...
  if (length(site_covariates) != length(site_data))
    stop("site_covariates must have one entry per site.")
  site_stats <- Map(function(d, covs) local_ipw_nocloning(
    d, tau = tau, t_star = t_star, hnum = hnum, trunc = trunc,
    covariates = covs), site_data, site_covariates)
...
local_landmark_ipw <- function(site_data, tau, t_star, p_num,
  ...
  if (!all(covariates %in% WEIGHT_COVARIATES))
    stop("Unknown landmark-model covariate(s).")
  ps_fit <- glm(reformulate(covariates, response = "A"),
...
run_fed_landmark_ipw <- function(dat, tau, t_star,
  ...
  if (length(site_covariates) != length(site_data))
    stop("site_covariates must have one entry per site.")
  site_stats <- Map(function(d, covs) local_landmark_ipw(
    d, tau = tau, t_star = t_star, p_num = p_num, trunc = trunc,
    covariates = covs), site_data, site_covariates)
...
run_local_ccw_meta <- function(dat, tau, t_star,
  ...
  if (length(site_covariates) != length(site_data))
    stop("site_covariates must have one entry per site.")
  site_summaries <- Map(function(d, covs) local_ccw_curve_summary(
    d, tau = tau, t_star = t_star, hnum = hnum, trunc = trunc,
    covariates = covs), site_data, site_covariates)
