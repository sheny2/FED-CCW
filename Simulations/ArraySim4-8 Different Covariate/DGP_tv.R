# =============================================================
# ArraySim4-8 discrete-time DGP with site-specific treatment-confounder sets.
#
# Interval m = 1..t_star.
...
simulate_multisite_tv <- function(K          = 3,
  ...
  if (!is.null(beta_init_by_site) && length(beta_init_by_site) != K)
    stop("beta_init_by_site must be NULL or have length K.")
  if (is.null(site_mix)) site_mix <- homogeneous_site_mix(K)
  validate_site_mix(site_mix, K)

  sites <- lapply(seq_len(K), function(k) {
    site_beta_init <- if (is.null(beta_init_by_site)) list(...)$beta_init else
      beta_init_by_site[[k]]
    if (is.null(site_beta_init)) site_beta_init <- DEFAULT_BETA_INIT
    if (!all(names(DEFAULT_BETA_INIT) %in% names(site_beta_init)))
      stop("Each site beta_init must include: ",
           paste(names(DEFAULT_BETA_INIT), collapse = ", "))
    ...
    do.call(simulate_site_tv, c(list(
                     ...
                     site_id = k,
                     seed    = base_seed + k,
                     x1_mean = site_mix$x1_mean[k],
                     x1_sd   = site_mix$x1_sd[k],
                     x2_prob = site_mix$x2_prob[k],
...
compute_truth_ccw_tv <- function(N          = 2e6,
                                 ...
                                 beta_init  = DEFAULT_BETA_INIT,
                                 beta_init_by_site = NULL,
                                 site_sizes = STUDY_SITE_SIZES,
                                 init_intercepts = STUDY_INIT_INTERCEPTS,
                                 site_mix   = homogeneous_site_mix(length(site_sizes)),
                                 beta_L     = DEFAULT_BETA_L,
                                 sd_L       = DEFAULT_SD_L,
                                 seed       = 999) {
  ...
  if (is.null(beta_init_by_site))
    beta_init_by_site <- rep(list(beta_init), length(site_sizes))
  if (length(beta_init_by_site) != length(site_sizes))
    stop("beta_init_by_site must have one entry per site.")
  if (!all(vapply(beta_init_by_site, function(x)
    all(names(DEFAULT_BETA_INIT) %in% names(x)), logical(1))))
    stop("Every site-specific beta_init vector is incomplete.")
  validate_site_mix(site_mix, length(site_sizes))
  ...
  init_coef <- function(term) vapply(beta_init_by_site, `[[`, numeric(1), term)[site]
  init_x1_i <- init_coef("x1")
  init_x2_i <- init_coef("x2")
  init_L1_i <- init_coef("L1")
  init_L2_i <- init_coef("L2")

  x1 <- rnorm(N, site_mix$x1_mean[site], site_mix$x1_sd[site])
  x2 <- rbinom(N, 1, site_mix$x2_prob[site])
  ...
  for (m in seq_len(M)) {
    ...
    not_yet <- alive & !initiated
    if (any(not_yet)) {
      lin_init <- init_intercept_i + init_x1_i * x1 + init_x2_i * x2 +
        init_L1_i * L1_m + init_L2_i * L2_m
  ...
  list(
    ...
    beta_init_by_site = beta_init_by_site,
    site_mix = site_mix
  )
}
