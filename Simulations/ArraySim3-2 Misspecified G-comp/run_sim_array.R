...
# multiplies the number of tasks, so it defaults to the correct model plus
# the two omitted-covariate variants; add "coarse_L" / "nonlinear_L" here to
# widen the sweep. The IPCW and per-protocol methods do not use an outcome
# model and are unaffected by this axis -- their results are re-estimated in
# each misspec cell and should agree up to Monte Carlo noise, which doubles
# as a sanity check.
GCOMP_MISSPEC_GRID <- c("correct", "no_lag", "no_tv", "coarse_L", "nonlinear_L")
...
# The SLURM array must cover 1..n_combos. If the array directive is stale
# (e.g. left at the pre-misspec 27) only the first block of the grid runs
# and whole misspec levels are silently skipped, so make the requirement
# loud on every task.
