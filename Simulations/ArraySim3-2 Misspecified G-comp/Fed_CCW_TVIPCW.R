...
.gcomp_fit <- function(dat, tau, t_star, clone_censor = TRUE,
  ...
  # dictated by `misspec`, and gated treatment. The family/link is also
  # spec-dependent (logit by default; cloglog or identity for the
  # link-misspecification variants).
  out_fit <- suppressWarnings(
    glm(.gcomp_outcome_formula(misspec),
        data = long, family = .gcomp_outcome_family(misspec))
  )
...
.gcomp_strategy_survival <- function(dat, fit, S_arm, t_star, mc = 20,
  ...
  haz <- function(m, x1, x2, L1, L2, L1lag, L2lag, trt) {
    ...
    # clamp to a valid hazard: the linear probability model can predict
    # outside [0,1], which would make (1 - haz) invalid and the survival
    # cumprod nonsensical. Clamping degrades LPM gracefully rather than
    # producing NaNs; for logit/cloglog it is a no-op since those are
    # already in (0,1).
