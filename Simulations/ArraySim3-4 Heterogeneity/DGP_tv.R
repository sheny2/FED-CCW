...
#   Oracle for the equal-sized mixture of heterogeneous sites.  Each site
#   uses its own baseline patient distribution and the true initiation
#   probabilities.  Site-specific marginal survival curves are averaged,
#   which is the standardized survival curve for the equally weighted target
#   population used by simulate_multisite_tv().
# =============================================================
compute_truth_multisite_tv <- function(N             = 3e6,
  ...
  truths <- lapply(seq_len(K), function(k) {
    compute_truth_ccw_tv(
      N          = N_site,
      tau        = tau,
      t_star     = t_star,
  ...
  S1 <- Reduce(`+`, lapply(truths, `[[`, "S1")) / K
  S0 <- Reduce(`+`, lapply(truths, `[[`, "S0")) / K
  M <- t_star
  psi1 <- 1 - S1[M + 1L]
  psi0 <- 1 - S0[M + 1L]
  RMST1 <- sum(S1[seq_len(M)])
