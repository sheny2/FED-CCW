...
# Joint implementation used by ArraySim4-7. It fits each local denominator
# model once, then feeds the exact same local weighted event/risk summaries to
# the two competing aggregation rules. This both reduces computation and
# guarantees that any fed-vs-meta difference is caused by aggregation rather
# than a second nuisance-model fit.
run_fed_and_local_ccw <- function(dat, tau, t_star,
                                  trunc = DEFAULT_TRUNC) {
  site_data <- split(dat, dat$site)
  hnum <- central_common_num_hazard(
    lapply(site_data, local_initiation_counts, t_star = t_star)
  )
  stats <- lapply(site_data, local_ccw_tvipcw,
                  tau = tau, t_star = t_star,
                  trunc = trunc, hnum = hnum)

  fed <- central_ccw_tvipcw(stats, tau = tau, t_star = t_star)
  site_summaries <- Map(function(s, n) {
    lambda1 <- ifelse(s$rw1 > 0, s$dw1 / s$rw1, 0)
    lambda0 <- ifelse(s$rw0 > 0, s$dw0 / s$rw0, 0)
    list(
      n = n,
      S1 = c(1, cumprod(1 - lambda1)),
      S0 = c(1, cumprod(1 - lambda0))
    )
  }, stats, vapply(site_data, nrow, integer(1)))
  local_meta <- central_ccw_curve_meta(site_summaries, t_star = t_star)

  list(fed = fed, local_meta = local_meta, stats = stats,
       hnum = hnum, site_summaries = site_summaries)
}
