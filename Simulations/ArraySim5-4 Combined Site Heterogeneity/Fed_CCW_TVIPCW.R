...
.tv_prep <- function(d, t_star) {
  ...
  list(n  = nrow(d), M = M,
       ...
       x1 = d$x1, x2 = d$x2, site = d$site, S = d$S,
...
local_initiation_counts <- function(site_data, t_star) {
  list(
    initiated = vapply(seq_len(t_star), function(m) {
      sum(site_data$S == m & site_data$T_event >= m)
    },
                       numeric(1)),
    eligible = vapply(seq_len(t_star), function(m) {
      sum(site_data$S >= m & site_data$T_event >= m)
    },
                      numeric(1))
  )
...
.tv_init_hazard <- function(P, hnum = NULL) {
  ...
  # Person-interval rows include only intervals in which the participant is
  # alive at the start and has not initiated before that interval. Because an
  # event occurs after the initiation decision, T_event itself is included.
  last_m <- pmin(P$S, P$Tev, M)
  ...
  pi_df <- data.frame(
    ...
    L2lag = L2lag[keep],
    id    = rows_id,
    site  = factor(P$site[rows_id])
  )
  pi_df$m <- droplevels(pi_df$m)

  den_formula <- if (nlevels(pi_df$site) > 1L) {
    y ~ m + site + x1 + x2 + L1 + L2 + L1lag + L2lag
  } else {
    y ~ m + x1 + x2 + L1 + L2 + L1lag + L2lag
  }
  den_fit <- glm(den_formula,
...
# -------------------------------------------------------------
# COMPARATOR: aligned, fully site-stratified pooled CCW.
#   The central analyst has all individual-level data, but deliberately fits
#   exactly the same separate initiation-hazard model within each site as
#   Fed-CCW. The common numerator, truncation rule, clone censoring, weighted
#   risk/event totals, survival estimator, and influence-function formulas
#   are also identical. Pooling the site-stratified weighted outcome records
#   is algebraically equivalent to summing the site risk/event totals below.
#
#   Consequently this is an implementation gold standard: point estimates
#   and model-based SEs should agree with Fed-CCW up to numerical precision.
# -------------------------------------------------------------
run_pooled_ccw_site_stratified <- function(dat, tau, t_star,
                                            trunc = DEFAULT_TRUNC) {
  hnum <- .tv_common_num_hazard(dat, t_star)
  stats <- lapply(split(dat, dat$site), local_ccw_tvipcw,
                  tau = tau, t_star = t_star, trunc = trunc, hnum = hnum)
...
# -------------------------------------------------------------
# COMPARATOR: conventional pooled CCW with site fixed effects.
#   One pooled person-interval logistic model is fitted to all sites:
#     logit P(initiation at m | eligible, history, site)
#       = interval fixed effects + site fixed effects + common slopes.
#   Passing all data to local_ccw_tvipcw() triggers that pooled model through
#   .tv_init_hazard(); weighted outcome records are then analyzed together.
# -------------------------------------------------------------
run_pooled_ccw_site_fe <- function(dat, tau, t_star,
                                    trunc = DEFAULT_TRUNC) {
  hnum <- .tv_common_num_hazard(dat, t_star)
  stats <- list(local_ccw_tvipcw(
    dat, tau = tau, t_star = t_star, trunc = trunc, hnum = hnum
  ))
  central_ccw_tvipcw(stats, tau = tau, t_star = t_star)
}

# Backward-compatible name: the default pooled comparator remains the aligned
# fully site-stratified implementation.
run_pooled_ccw_tvipcw <- run_pooled_ccw_site_stratified
