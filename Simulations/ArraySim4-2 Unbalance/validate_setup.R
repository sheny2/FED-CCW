#!/usr/bin/env Rscript
# Lightweight structural and end-to-end preflight for ArraySim4-2 Unbalance.
...
dat <- simulate_multisite_tv(
  K = 3, n_per_site = UNBALANCED_SITE_SIZES,
  init_intercepts = SITE_INIT_INTERCEPTS,
...
stopifnot(identical(as.integer(site_n), as.integer(UNBALANCED_SITE_SIZES)))
init_rate <- tapply(dat$A_tau, dat$site, mean)
stopifnot(init_rate[[1L]] < init_rate[[2L]], init_rate[[2L]] < init_rate[[3L]])
message(sprintf(
  "PASS: unbalanced site design generated (n=%s; initiation by tau=%.3f/%.3f/%.3f).",
  paste(site_n, collapse = "/"), init_rate[1], init_rate[2], init_rate[3]
))
...
# With one site, local CCW curve meta-analysis reduces exactly to Fed-CCW.
meta1 <- run_local_ccw_meta(one_site, tau, t_star)
stopifnot(
  abs(fed1$RD - meta1$RD) < 1e-12,
  abs(fed1$RMST_diff - meta1$RMST_diff) < 1e-12
)
message("PASS: K=1 local CCW meta-analysis reduces to Fed-CCW.")

# Landmark follow-up must begin only after tau.
landmark <- run_fed_landmark_ipw(dat, tau, t_star)
stopifnot(
  all(landmark$S1[seq_len(tau + 1L)] == 1),
  all(landmark$S0[seq_len(tau + 1L)] == 1),
  all(is.finite(unlist(landmark[c("RD", "RR", "OR", "RMST_diff")])))
)
message("PASS: landmark IPW starts follow-up after tau and is finite.")
...
truth <- compute_truth_ccw_tv(
  ...
  beta_event = set_beta_trt(-0.7),
  beta_init = scale_confounding(DEFAULT_CONF_MULTS[["strong"]]),
  site_sizes = UNBALANCED_SITE_SIZES,
  init_intercepts = SITE_INIT_INTERCEPTS,
  seed = 24680
)
out <- run_once_tv(
...
message("PASS: end-to-end run returns exactly six methods and no G-comp.")
message("All ArraySim4-2 Unbalance preflight checks passed.")
