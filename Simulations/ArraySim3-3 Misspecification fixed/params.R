...
#
# This four-method study compares federated CCW, pooled CCW, direct
# g-computation, and clone-censor g-computation. A common specification axis
# is applied to the CCW weight denominator and the g-computation outcome
# model, so all four methods see the same kind of nuisance-model change.
# =============================================================
...
# A single axis changes BOTH nuisance models: the g-computation outcome
# hazard and the CCW initiation-hazard denominator.
#
...
#               its fitting-sample median.
#   no_tv    -- drop all current and lagged L terms.
#   tree     -- retain the full covariate set, but replace the logistic GLM
#               with an rpart classification tree. For CCW this replaces the
#               denominator model only; its marginal stabilizing numerator
#               remains logistic. For g-computation it replaces the outcome
#               hazard model. L transition models remain correctly specified.
#
...
MISSPEC_SPECS  <- c("correct", "coarse_L", "no_tv", "tree")
DEFAULT_MISSPEC <- "correct"

# Fixed across methods so the tree scenario is comparable between federated,
# pooled, direct-gcomp, and CC-gcomp fits. xval = 0 avoids internal
# cross-validation inside every simulation replicate.
TREE_CONTROL <- list(cp = 0.001, minsplit = 50, minbucket = 20,
                     maxdepth = 8, xval = 0)
...
.tv_block <- function(misspec = DEFAULT_MISSPEC) {
  switch(misspec,
    ...
    no_tv    = "",
    tree     = "L1 + L2 + L1lag + L2lag",
    stop("Unknown misspecification '", misspec, "'. Options: ",
         paste(MISSPEC_SPECS, collapse = ", "))
  )
}
