#!/usr/bin/env Rscript
# ============================================================================
# ArraySim5-2 Patient Mix Heterogeneity array-job simulation over the
# (tau, beta_trt, patient-mix heterogeneity) grid.
#
# The confounding and outcome coefficients are fixed. Across the low,
# moderate and high scenarios, only the site-specific distributions of X1
# and X2 change. The oracle is therefore recomputed for every grid cell.
#
...
# ---- 1. parameter grid --------------------------------------------------
# Patient-mix scenarios come from DEFAULT_PATIENT_MIX in params.R.
...
SIM <- list(
  ...
  patient_mix = get_patient_mix(this$heterogeneity),
...
message(sprintf("[task %d/%d] tau=%g  beta_trt=%g  patient-mix heterogeneity=%s",
...
message("  patient mix: ", paste(
  sprintf("site %d: X1~N(%.2f, %.2f^2), P(X2=1)=%.2f",
...
results$patient_mix <- paste(
  sprintf("s%d:N(%.2f,%.2f^2),Bern(%.2f)", SIM$patient_mix$site,
