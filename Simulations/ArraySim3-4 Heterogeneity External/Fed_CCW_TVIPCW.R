...
#   numerator comes from an independent external reference cohort and is
#   fixed before analysis. Every site therefore targets the same strategy
...
# ---- fixed external numerator validation --------------------------------
.validate_external_hnum <- function(hnum, t_star) {
  if (missing(hnum) || is.null(hnum))
    stop("A fixed external numerator vector is required.")
  if (length(hnum) < t_star)
    stop("External numerator must cover every follow-up interval.")
  hnum <- as.numeric(hnum[seq_len(t_star)])
  if (any(!is.finite(hnum)) || any(hnum <= 0 | hnum >= 1))
    stop("External numerator hazards must be in (0, 1).")
  hnum
}

# ---- per-site initiation-hazard models ---------------------------------
# Denominator conditions on X, current L_m and lagged L_{m-1}, matching the
# outcome hazard's dependence on current + lagged L. `hnum` is the fixed
# external-reference numerator known to every site before analysis.
...
run_fed_ccw_tvipcw <- function(dat, tau, t_star, hnum,
                               trunc = DEFAULT_TRUNC) {
  hnum <- .validate_external_hnum(hnum, t_star)
...
# -------------------------------------------------------------
run_pooled_ccw_tvipcw <- function(dat, tau, t_star, hnum,
                                  trunc = DEFAULT_TRUNC) {
  hnum <- .validate_external_hnum(hnum, t_star)
  pooled <- dat
  pooled$site <- 1L
...
#   estimates only.
# -------------------------------------------------------------
run_tvipcw_nocloning <- function(dat, tau, t_star, hnum,
                                 trunc = DEFAULT_TRUNC) {
  M <- t_star
  hnum <- .validate_external_hnum(hnum, t_star)
...
# =============================================================
# DIRECT G-COMPUTATION (no cloning)
# =============================================================
#   Fits a pooled discrete-time outcome hazard and pooled L-transition models
#   on observed follow-up, then standardizes by reconstructing each strategy's
#   covariate trajectory (Monte Carlo) and averaging person-specific survival
#   curves.  This is the direct/no-cloning version requested for ArraySim3-4;
#   the clone-censor ("indirect") g-computation variant is intentionally not
#   included.
#
...
# -------------------------------------------------------------
# DIRECT G-COMPUTATION (no cloning, no censoring)
