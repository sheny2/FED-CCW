#!/usr/bin/env Rscript
# Aggregate ArraySim4-4 factorial results and create interpretable plots.
...
req_cols <- c(
  ...
  "beta_trt", "scenario", "conf_mult", "sample_size_scenario",
  "initiation_scenario", "n_sites", "n_per_site", "total_n",
  "init_intercept", "zero_cov_init_prob", "observed_init_rate",
  "min_site_init_rate", "max_site_init_rate", "landmark_n",
  "landmark_treated_rate", "task_id"
)
...
cell_counts <- results %>%
  count(task_id, method, estimand)
if (any(cell_counts$n != 300L))
  warning("At least one task/method/estimand cell does not have 300 replicates.")

# Preserve all eight estimands in the summary. Monte Carlo RMSE is calculated
# from replicate-level errors before collapsing.
summ <- results %>%
  group_by(
    method, estimand, tau, beta_trt, scenario, conf_mult,
    sample_size_scenario, initiation_scenario, n_sites, n_per_site, total_n,
    init_intercept, zero_cov_init_prob
  ) %>%
  summarise(
    ...
    truth = first(truth),
    bias = mean(bias),
    median_bias = median(estimate) - first(truth),
    emp_sd = sd(estimate),
    rmse = sqrt(mean(bias^2)),
    coverage = if (all(is.na(covered))) NA_real_ else mean(covered, na.rm = TRUE),
    mean_observed_init_rate = mean(observed_init_rate),
    mean_min_site_init_rate = mean(min_site_init_rate),
    mean_max_site_init_rate = mean(max_site_init_rate),
    mean_landmark_n = mean(landmark_n),
    mean_landmark_treated_rate = mean(landmark_treated_rate, na.rm = TRUE),
    n_reps = n(),
    .groups = "drop"
  ) %>%
  arrange(estimand, method, initiation_scenario, sample_size_scenario, tau) %>%
  as.data.frame()
write.csv(summ, "summary_all_scenarios.csv", row.names = FALSE)
message("Wrote summary_all_scenarios.csv")

# One row per replicate/design cell for achieved treatment and landmark sizes.
design_diagnostics <- results %>%
  distinct(
    task_id, rep, tau, sample_size_scenario, initiation_scenario,
    n_per_site, total_n, init_intercept, zero_cov_init_prob,
    observed_init_rate, min_site_init_rate, max_site_init_rate,
    landmark_n, landmark_treated_rate
  ) %>%
  arrange(task_id, rep) %>%
  as.data.frame()
...
# Track rare but consequential ratio explosions explicitly.
ratio_extremes <- results %>%
  filter(estimand %in% c("RR", "OR")) %>%
  group_by(method, estimand, tau, sample_size_scenario, initiation_scenario) %>%
  summarise(
    n_reps = n(),
...
plot_data <- results %>%
  ...
  mutate(
    ...
    sample_f = factor(
      sample_size_scenario, levels = c("low", "large"),
      labels = c("100 patients/site", "1,000 patients/site")
    ),
    initiation_f = factor(
...
for (est in c("RD", "RMST_diff")) {
  d <- plot_data %>% filter(estimand == est) %>% mutate(error = bias)
  p <- ggplot(d, aes(x = tau_f, y = error, fill = method)) +
    geom_boxplot(outlier.size = 0.4, alpha = 0.9,
                 position = position_dodge(width = 0.8), linewidth = 0.25) +
    geom_hline(yintercept = 0, linetype = "dashed", linewidth = 0.4) +
    facet_grid(initiation_f ~ sample_f) +
    scale_fill_manual(values = method_colors, drop = FALSE) +
    labs(
      title = sprintf("Bias of %s: sample size x initiation prevalence", est),
      subtitle = "300 reps/cell | medium confounding | beta=-0.7 | t*=25 | 10 sites",
...
# A log estimate/truth error keeps rare landmark explosions visible without
# flattening every other RR/OR boxplot against zero. Zero remains ideal.
for (est in c("RR", "OR")) {
  d <- plot_data %>%
    filter(estimand == est, estimate > 0, truth > 0) %>%
    mutate(error = log(estimate / truth))
  p <- ggplot(d, aes(x = tau_f, y = error, fill = method)) +
    geom_boxplot(outlier.size = 0.4, alpha = 0.9,
                 position = position_dodge(width = 0.8), linewidth = 0.25) +
    geom_hline(yintercept = 0, linetype = "dashed", linewidth = 0.4) +
    ...
    scale_fill_manual(values = method_colors, drop = FALSE) +
    labs(
      title = sprintf("Log-ratio error of %s: sample size x initiation prevalence", est),
      subtitle = paste(
        "log(estimate/truth); zero is ideal | 300 reps/cell |",
        "medium confounding | beta=-0.7 | t*=25 | 10 sites"
      ),
