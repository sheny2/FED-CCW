...
#   broadcast to every site, ensuring a common target under heterogeneity.
#
...
.tv_common_num_hazard <- function(dat, t_star) {
  local_counts <- lapply(split(dat, dat$site), function(d) {
    list(
      initiated = vapply(seq_len(t_star), function(m) sum(d$S == m),
                         numeric(1)),
      eligible = vapply(seq_len(t_star), function(m) sum(d$S >= m),
                        numeric(1))
    )
  })
