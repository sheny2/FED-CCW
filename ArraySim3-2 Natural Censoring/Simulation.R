# =============================================================
# Simulation.R -- one replicate of the natural-censoring study:
#   fed-CCW-TVIPCW, pooled-CCW-TVIPCW, CC-gcomp, per-protocol (naive),
#   IPCW-without-cloning (naive). All correctly specified.
# =============================================================
...
run_once_tv <- function(K          = 3,
  ...
  fits <- list(
    fed_ccw_tvipcw    = run_fed_ccw_tvipcw(dat, tau = tau, t_star = t_star,
                                           trunc = trunc),
    pooled_ccw_tvipcw = run_pooled_ccw_tvipcw(dat, tau = tau, t_star = t_star,
                                              trunc = trunc),
    ccw_gcomp         = run_ccw_gcomp(dat, tau = tau, t_star = t_star,
                                      mc = mc_gcomp, seed = base_seed + 777),
    tvipcw_no_clone   = run_tvipcw_nocloning(dat, tau = tau, t_star = t_star,
                                             trunc = trunc),
    perprotocol_naive = run_perprotocol_noiptw(dat, tau = tau, t_star = t_star)
  )
