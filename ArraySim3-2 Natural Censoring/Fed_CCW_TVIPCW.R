...
# g=1: deviate at tau if still uninitiated at tau -> censor at tau.
...
# =============================================================
#   After cloning and artificial censoring, do not reweight. Fit pooled
#   transition models for L1, L2 and a pooled outcome hazard on the cloned/
#   censored person-interval data, reconstruct each strategy's covariate
#   trajectory by Monte Carlo, and standardize.
#
#   NATURAL CENSORING needs no model here: censored person-intervals simply
#   contribute fewer rows to the fits; the forward simulation is unaffected.
#   This is the methodological contrast with IPCW (which needs a correct
#   censoring model) -- neither dominates, they rely on different models.
#
#   NOT FEDERATED: nuisance models are fit on pooled person-interval data.

# ---- pooled person-interval frame (rows to observed follow-up) ----------
.gcomp_long <- function(dat, tau, t_star) {
  ...
  Tobs <- pmin(dat$T_event, dat$T_cens, t_star)
  ...
  data.frame(
    ...
    L1lag = .lag_matrix(L1)[keep], L2lag = .lag_matrix(L2)[keep],
...
.gcomp_fit <- function(dat, tau, t_star) {
  M    <- t_star
  ...
  # L transition models (linear-Gaussian, no treatment term); m >= 2 only.
  tr <- long[long$m >= 2, ]
  L1_fit <- lm(L1 ~ L1lag + x1 + x2, data = tr)
  L2_fit <- lm(L2 ~ L2lag + x1 + x2, data = tr)

  list(out_fit = out_fit, L1_fit = L1_fit, L2_fit = L2_fit,
       sd1 = sd(residuals(L1_fit)), sd2 = sd(residuals(L2_fit)), M = M)
}
...
.gcomp_strategy_survival <- function(dat, fit, S_arm, t_star, mc = 20,
  ...
  haz <- function(m, x1, x2, L1, L2, L1lag, L2lag, trt) {
    nd <- data.frame(m = m, x1 = x1, x2 = x2,
                     L1 = L1, L2 = L2, L1lag = L1lag, L2lag = L2lag, trt = trt)
    predict(b_out, newdata = nd, type = "response")
  }

  Ssum <- numeric(M + 1); Ssum[1] <- n * mc

  for (i in seq_len(n)) {
    x1 <- dat$x1[i]; x2 <- dat$x2[i]; Son <- S_arm[i]
    surv_i <- matrix(1, mc, M + 1)
    L1prev <- rep(0, mc); L2prev <- rep(0, mc); runS <- rep(1, mc)

    for (m in seq_len(M)) {
      trt <- as.integer(m >= Son)
      mu1 <- b1["(Intercept)"] + b1["L1lag"] * L1prev + b1["x1"] * x1 + b1["x2"] * x2
      mu2 <- b2["(Intercept)"] + b2["L2lag"] * L2prev + b2["x1"] * x1 + b2["x2"] * x2
      L1m <- rnorm(mc, mu1, fit$sd1); L2m <- rnorm(mc, mu2, fit$sd2)
