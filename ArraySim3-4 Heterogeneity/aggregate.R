...
req_cols <- c("heterogeneity", "tau", "beta_trt", "t_star", "task_id")
...
method_levels <- c(
  "fed_ccw_tvipcw", "pooled_ccw_tvipcw", "direct_gcomp",
  "tvipcw_no_clone", "perprotocol_naive"
)
method_labels <- c(
  "Federated CCW", "Pooled CCW", "Direct G-comp",
  "TV-IPCW (no cloning)", "Per-protocol (naive)"
)
method_colors <- c(
  "Federated CCW"         = "#2c7fb8",
  "Pooled CCW"            = "#41b6c4",
  "Direct G-comp"         = "#568203",
...
summ <- pdat %>%
  group_by(method, estimand, tau, beta_trt, heterogeneity, t_star) %>%
  summarise(
