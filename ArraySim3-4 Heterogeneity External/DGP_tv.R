...
#   Oracle aligned with the one-shot external-numerator estimator:
#     1. simulate each heterogeneous site using the true DGP;
#     2. use the fixed numerator from an independent reference cohort;
...
compute_truth_multisite_tv <- function(N             = 3e6,
  ...
  M <- t_star
  if (missing(numerator_hazard) || is.null(numerator_hazard))
    stop("The oracle requires the fixed external numerator.")
  if (length(numerator_hazard) < tau)
    stop("External numerator does not cover the grace period.")
  hnum <- as.numeric(numerator_hazard[seq_len(tau)])
